...
namespace RSPGame // Note: actual namespace depends on the project name.
{
    class Program
    {
        const int REWARD = 10;
        static void Main(string[] args)
        {
            ...
            while (playing)
            {
                ...
                DisplayResults(result, balance);
                playing = IsEnoughToPlay(balance) && IsContinue();
            }
            EndGameDisplay(balance);
        ...
        static void DisplayResults(string result, int balance)
        {
            Console.WriteLine("Game result: " + result);
            Console.WriteLine("Your balance is now " + balance);
        }
        ...
        static void EndGameDisplay(int balance)
        {
